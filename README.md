...
LOTIS (Low Time Script) is a framework for rapid development of interactive web applications and business systems in PHP.  
You build applications using a desktop-like approach with OOP principles — without tight coupling to frontend markup.

## Key Features

- 💻 **Desktop-like approach in WEB**: Control the interface through PHP objects (`Form`, `DataTable`) — no manual HTML/CSS required.
- 🔄 **Unified namespace**: Access objects through the same variables on both server and client sides.
- 🗃 **Built-in ORM**: Database operations via `MySqlTable` and `QueryBuilder` classes.
- 📦 **Ready-to-use business components**: documents with line items (`DataView`), data stocks (`Stock`), real-time database sync (`DataSync`) — for complex enterprise systems.
- ⚡ **Event-driven model**: `Events` replaces AJAX — you write logic, not HTTP requests.
- 🧩 **Minimal dependencies**: Just PHP, jQuery, and MySQL.
...
- PHP classes generate metadata.
- The `Space` class transforms metadata into HTML.
...
Or simply include the `src/newlotis` folder in your project.
...
A practical guide to engineering thinking in development, based on 25 years of experience building commercial web applications.  
The author offers a unique approach: instead of learning ready-made solutions, you will build your own framework, understanding every line of code from the inside out.

*Note: Currently available in Russian only.*
...
- ✅ PHP 7+
- ✅ No Composer required (but supported)
- ✅ Lightweight (~50 KB)
- ✅ Handles up to 10,000 rows in tables
...
├── LTS/                 # Core
│   ├── DataView.php     # Main form component
│   ├── DataTable.php    # Tables with sorting and filtering
│   ├── FilterForm.php   # Table filter forms
│   ├── Stock.php        # Centralized accounting/ledger
│   ├── Space.php        # UI assembly
│   ├── Events.php       # AJAX events handler
│   ├── Form.php         # Input form: base container for fields
│   ├── Input.php        # Text input field
│   ├── LookupField.php  # Search field with dropdown and autoload
│   ├── JS.php           # JavaScript logic generation on PHP side
│   ├── CSS.php          # CSS injection and runtime management
...
│   ├── Accordion.php    # Collapsible blocks for form grouping
│   ├── Construct.php    # Dynamic UI structure creation
│   ├── DataSync.php     # Client-server data synchronization
│   ├── Debug.php        # Debugging tools: logging, variable output
│   ├── Element.php      # Base class for all UI elements
│   ├── Ether.php        # Pub/sub messaging system
│   ├── Html.php         # HTML tags: label, span, div, etc.
...
│   └── Video.php        # Video embedding (local or remote)
├── JS/                  # Client-side
│   ├── lts.js           
│   ├── Form.js          
│   ├── Events.js        
